Brebcoin Core version *0.15.1* is now available from:

  <https://download.brebcoin.org/brebcoin-0.15.1/>
...
A number of changes to the way Brebcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 bred per kB (0.00001 BREB). This is to prepare for dropping the minimum transaction fee to 0.1 bred per kB (0.0001 BREB) in the next release.
